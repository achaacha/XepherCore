...
namespace XepherBot.Core.Common
{
    ...
    {
        public static T ParseFrom<T>(string[] args) where T : IXepherCommandOptions, new()
            => ParseFrom(new T(), args).Item1;

        public static (T, bool) ParseFrom<T>(T options, string[] args) where T : IXepherCommandOptions
        {
